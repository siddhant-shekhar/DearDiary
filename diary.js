diary = {
    ...
    showEntries: function() {
        ...
        if (!data) {
            data = [
                ...
                "<div class='date'>Tue Mar 17 01:05 2022</div>" +
     ...
     deleteEntry:function(subject,body) { 
         var data = localStorage.getItem("data");
        if (data) data = JSON.parse(data);
        else data = [];
        body = body.replace(/\n/g, "<br/>");
        var $cont = $("<div></div>");
        $("<h3></h3>").html(subject).appendTo($cont);
        $("<div class='date'></div>").text((new Date).toLocaleString()).appendTo($cont);
        $("<p></p>").html(body).appendTo($cont);
      // $("<button></button>").text('Delete').appendTo($cont);
    ...
    okEdit: function () {
        ...
        diary.deleteEntry(subject,body);
        diary.getLoc();
        diary.cancelEdit();
    },
